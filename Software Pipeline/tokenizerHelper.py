...
def cleanCode(raw_code):
    ...
    lines = [line.split("//")[0].strip() for line in lines if is_valid(line)]
    return lines
...
def handleTokenCandidate(candidate):
    if not candidate:
        return []
    ret = []
    match = re.search(r"([\&\|\(\)<=\+\-\*>\\/.;,\[\]}{~])", candidate.strip())
    if match is not None:
        ret.extend(handleTokenCandidate(match.string[: match.start()]))
        ret.append(match.string[match.start()])
        ret.extend(handleTokenCandidate(match.string[match.end() :]))
    else:
