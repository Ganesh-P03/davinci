...
class CodeWriter:
  ...
  __reg = 18 # Use reg instead of stack
  ...
  def writeArithmetic(self, command: str) -> None:
    ...
    if command == 'not' or command == 'neg':
      if command == 'neg':
        self.writeMessage('Negate')
        self.write(f'sub {reg2}, $zero, {reg2}')  # t0 = 0 - x (-x)
      else:
        self.writeMessage('Not')
        self.write(f"sub {reg2}, $zero, {reg2}")  # t0 = 0 - x (-x)
    ...
    # Eq/Gt/Lt
      ...
      self.writeMessage('Greater than')
      self.write(f'slt {reg1}, {reg1}, {reg2}') # t0 = (y < x) => x > y
    elif command == 'lt':
      self.writeMessage('Less than')
      self.write(f'slt {reg1}, {reg2}, {reg1}') # t0 = x < y
    elif command == "mac":
      self.writeMessage('MAC')
      self.write(f'mac {reg1} {reg2}')
      self.write(f'add {reg1} x1, $zero')
  ...
  def writePushPop(self, command: str, segment: str, index: int) -> None:
    ...
    if command_type == 'C_PUSH':
      ...
      if segment == 'argument':
        self.write(f'lw {reg1}, ' + str(index) + '($arg)') # t0 = *(arg + index)
      elif segment == 'local':
        self.write(f'lw {reg1}, ' + str(index) + '($lcl)') # t0 = *(lcl + index)
      elif segment == 'this':
        self.write('add $t1, $this, $ram')
        self.write(f'lw {reg1}, ' + str(index) + '($t1)') # t0 = *(this + index)
      elif segment == 'that':
        self.write('add $t1, $that, $ram')
        self.write(f'lw {reg1}, ' + str(index) + '($t1)') # t0 = *(that + index)
      elif segment == 'temp':
        self.write(f'lw {reg1}, ' + str(index) + '($temp)') # t0 = *(temp + index)
      elif segment == 'static':
        self.write(f'lw {reg1}, ' + str(self.__file_name) + '.' + str(index//4)) # t0 = *(filename.index)
        ...
        if upper_bits != 0:
          ...
          self.write(f'addi {reg1}, {reg1}, ' + str(lower_bits)) # t0 = lower_bits
        else:
          self.write(f'addi {reg1}, $zero, ' + str(lower_bits)) # t0 = lower_bits
      ...
      if segment == 'argument':
        self.write(f'sw {reg1}, ' + str(index) + '($arg)') # t0 = *(arg + index)
      elif segment == 'local':
        self.write(f'sw {reg1}, ' + str(index) + '($lcl)') # t0 = *(lcl + index)
      elif segment == 'this':
        self.write(f'add $t1, $this, $ram')
        self.write(f'sw {reg1}, ' + str(index) + '($t1)') # t0 = *(this + index)
      elif segment == 'that':
        self.write(f'add $t1, $that, $ram')
        self.write(f'sw {reg1}, ' + str(index) + '($t1)') # t0 = *(that + index)
      elif segment == 'temp':
        self.write(f'sw {reg1}, ' + str(index) + '($temp)') # t0 = *(temp + index)
      elif segment == 'static':
        self.write(f'sw {reg1}, ' + str(self.__file_name) + '.' + str(index//4)) # t0 = *(filename.index)
      elif segment == "pointer" and index == 0:
