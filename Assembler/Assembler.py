...
class Assembler:
  ...
  def translateRType(self, code: list) -> str:
    ...
    assert (cmd == 'add') or (cmd == 'sub') or (cmd == 'and') or (cmd == 'or') or (cmd == 'slt'), "Invalid command" + cmd
    assert (rd != '00000'), "Storing to zero register"
    assert (len(rs1) == 5 and len(rs2) == 5 and len(rd) == 5), "Invalid register length " + cmd
  ...
  def translateIType(self, code: list) -> str:
    ...
    if cmd == 'lw':
      ...
      assert (len(rs1) == 5 and len(rd) == 5), "Invalid register length " + cmd
      assert (len(imm) == 12), "Invalid immediate length " + cmd + " " + str(len(imm))
  ...
  def secondPass(self, save: bool = False):
    ...
    for pc, line in enumerate(self.__contents):
      ...
      # Replacing labels
      if (cmd == 'bne' or cmd == 'beq') or (cmd == 'addi' or cmd == 'lui') or (cmd == 'jal' or cmd == 'jalr'):
        if cmd == 'addi':
  ...
  def translate(self):
    ...
    for pc, code in enumerate(self.__contents):
      ...
      elif (cmd == 'add') or (cmd == 'sub') or (cmd == 'and') or (cmd == 'or') or (cmd == 'slt'):
